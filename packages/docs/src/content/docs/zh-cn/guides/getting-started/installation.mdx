...
4.  启动开发服务器
    ...
    ![Prestore Data Panel](@assets/images/prestore-data-panel.png)
...
<Aside type="tip" class="mt-4">
  查看 [fuzzy 示例](/zh-cn/examples/fuzzy) 以更好的了解 fuzzy 机制。
</Aside>
...
当数据加载完成后，Gueleton 会移除占位骨架并显示实际内容。`container.leave` 选项允许你为 `container` 元素定义离开时的样式。
配合 `transition` 属性使用，可以实现更平滑的过渡效果。

[过渡动画示例](/zh-cn/examples/transition) 展示了通过修改 leave 样式来实现骨架淡出效果。
